...
SHANGYANG_ACTORS = Actors(
    author="content-author-shangyang",
...
def _assert_existing_shangyang_matches_source(
    *,
    root: Path,
    content_item: object,
) -> None:
    ...
    if content.draft_fingerprint(sealed) != content.draft_fingerprint(
        ...
        raise RuntimeError(
            "The published L103 package differs from the canonical source; "
            "publish a new immutable content version instead of silently "
            "accepting source drift."
        )
    ...
    canonical_scenario = build_shangyang_scenario_draft(
        build_shangyang_course_draft()
    )
    primary_descriptor = next(
        descriptor
    ...
    for field in (
        ...
        scenario_payload.pop(field, None)
    if scenario_payload != canonical_scenario.contract_content():
        raise RuntimeError("The published L103 scenario differs from its source.")

    canonical_evidence = build_shangyang_evidence_draft(
        build_shangyang_course_draft()
    )
    sealed_evidence = resources.evidence_corpus
    evidence_fields = (
    ...
    if evidence_payload != draft_payload:
        raise RuntimeError("The published L103 evidence differs from its source.")
    ...
    if not verify_contract_checksum(
        ...
        raise RuntimeError("The published L103 presentation differs from local media.")
...
def publish_shangyang(*, root: Path) -> dict[str, object]:
    content.configure(root)
    ...
    if current is not None:
        ...
        if item is not None:
            _assert_existing_shangyang_matches_source(root=root, content_item=item)
            resources = workflow.get_published_lesson_resources(
                draft_source.course_id,
                draft_source.lesson_id,
            )
    ...
    saved = content.save_draft(draft_source, saved_by=SHANGYANG_ACTORS.author)
    validated = workflow.validate_draft(
        saved.lesson_id,
        actor=SHANGYANG_ACTORS.author,
    )
    if validated.validation is None or not validated.validation.valid:
        raise RuntimeError("Shangyang course draft did not pass formal validation.")
    workflow.submit_for_review(
        saved.lesson_id,
        actor=SHANGYANG_ACTORS.author,
        note="L103 正文、史实边界、人物、地图与课堂结构已完成，申请独立审校。",
    )
    workflow.approve_draft(
        saved.lesson_id,
        actor=SHANGYANG_ACTORS.reviewer,
        note="已核对后世记载、同时代器物、较晚秦简、文本层次和制度代价，批准封存。",
    )
    sealed_course, _, _ = workflow.seal_approved_draft(
        saved.lesson_id,
        actor=SHANGYANG_ACTORS.publisher,
    )

    scenario_saved = scenario_authoring.save_scenario_draft(
        build_shangyang_scenario_draft(saved),
        saved_by=SHANGYANG_ACTORS.author,
    )
    scenario_report = scenario_authoring.validate_saved_scenario_draft(
        scenario_saved.scenario_id
    )
    if not scenario_report.valid:
        raise RuntimeError(
            "Shangyang scenario did not validate: "
            + "; ".join(item.message for item in scenario_report.issues)
        )
    scenario, scenario_record, _ = scenario_authoring.seal_scenario_draft(
        scenario_saved.scenario_id,
    ...
    evidence_saved, _ = evidence_workflow.save_evidence_draft(
        build_shangyang_evidence_draft(saved),
        saved_by=SHANGYANG_ACTORS.author,
    )
    _, evidence_report = evidence_workflow.validate_evidence_draft(
        evidence_saved.corpus_id,
    ...
    if not evidence_report.valid:
        raise RuntimeError("Shangyang evidence corpus did not pass formal validation.")
    evidence_workflow.submit_evidence_for_review(
        evidence_saved.corpus_id,
        actor=SHANGYANG_ACTORS.author,
        note="10 项来源与 30 个稳定片段已逐项绑定事实，申请独立审校。",
    )
    evidence_workflow.review_evidence_draft(
        evidence_saved.corpus_id,
        actor=SHANGYANG_ACTORS.reviewer,
        decision="approve",
        note="来源年代、文本层次、权利说明、人物边界和争议标记均已核对。",
    )
    evidence, evidence_record, _, _ = evidence_workflow.seal_approved_evidence(
        evidence_saved.corpus_id,
        actor=SHANGYANG_ACTORS.publisher,
    ...
    presentation = build_shangyang_presentation(
        root,
        sealed_by=SHANGYANG_ACTORS.publisher,
    )
    presentation_record = runtime_artifacts.stage_lesson_presentation(
    ...
    release, published_workflow = workflow.publish_version(
        ...
        sealed_course.version,
        actor=SHANGYANG_ACTORS.publisher,
        note="发布 L103 商鞅变法正式旗舰课：课程、节点式六回合关卡、证据库与导读资源精确绑定。",
        scenario_selections=[
            workflow.ScenarioReleaseSelection(
                scenario_id=scenario.scenario_id,
                scenario_version=scenario.scenario_version,
    ...
    if (
        ...
        or resources.lesson_presentation.checksum
        != presentation_record.descriptor.checksum
        or scenario_record.descriptor.artifact_id != SHANGYANG_SCENARIO_ID
        or evidence_record.descriptor.artifact_id != SHANGYANG_CORPUS_ID
...
def main() -> int:
    ...
    parser.add_argument("lesson", choices=("L101", "L103"))
    parser.add_argument(
        "--content-root",
        type=Path,
    ...
    publishers = {"L101": publish_dayu, "L103": publish_shangyang}
    result = publishers[args.lesson](root=args.content_root.resolve())
